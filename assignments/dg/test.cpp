...
public:
  RingIterConst(const RingQueue<T, capacity> &iterateOver, int offset,
                int increment = 1)
      : rq(iterateOver), off(offset), inc(increment) {}
  ~RingIterConst() {}
  bool operator==(const RingIterConst &i) {
    return &i.rq == &rq && i.off == off;
  }
  bool operator!=(const RingIterConst &i) { return !(*this == i); }
  thisclass &operator++() {
    off += inc;
    return *this;
  }
  ...
  typename std::iterator_traits<thisclass>::difference_type
  operator-(thisclass &sibling) const {
    return (off - sibling.off) / inc;
  }
  thisclass &operator+=(int amount) {
    off += (amount * inc);
    return *this;
  }
  thisclass &operator-=(int amount) {
...
class RingIter : public RingIterConst<T, capacity> {
public:
  RingIter(RingQueue<T, capacity> &iterateOver, int offset)
      : RingIterConst<T, capacity>(iterateOver, offset) {}
  ~RingIter() {}
  T &operator[](int index) {
    return this->rq.buf[(this->rq.begin_idx + this->off + (index * this->inc)) %
                        this->capacity];
  }
...
namespace std {
template <typename T, int capacity>
class iterator_traits<RingIterConst<T, capacity>> {
public:
...
class iterator_traits<RingIter<T, capacity>>
    : public iterator_traits<RingIterConst<T, capacity>> {
