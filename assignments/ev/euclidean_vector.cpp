...
EuclideanVector::EuclideanVector(int i, double j)
    : size_{i}, magnitudes_{make_unique<double[]>(i)} {
  for (int k = 0; k < i; k++) {
    magnitudes_[k] = j;
  }
}
...
EuclideanVector::EuclideanVector(const EuclideanVector &another)
    : size_{another.size_} {
  this->magnitudes_ = make_unique<double[]>(this->size_);
...
EuclideanVector::EuclideanVector(EuclideanVector &&another) noexcept
    : size_{another.size_}, magnitudes_{move(another.magnitudes_)} {
  another.size_ = 0;
}
...
EuclideanVector &EuclideanVector::operator+=(const EuclideanVector &rhs) {
  if (this->size_ != rhs.size_) {
    ...
    ss << "and RHS(" << rhs.size_ << ")"
       << " do not match";
    throw std::runtime_error(ss.str());
  }
  for (int i = 0; i < this->size_; i++) {
    this->magnitudes_.get()[i] =
        this->magnitudes_.get()[i] + rhs.magnitudes_.get()[i];
  }
  return *this;
}
...
EuclideanVector EuclideanVector::CreateUnitVector() {
  ...
  if (norm == 0) {
    stringstream ss;
    ss << "EuclideanVector with euclidean normal of 0 does not have a unit "
          "vector";
    throw std::runtime_error(ss.str());
  }
...
EuclideanVector operator+(const EuclideanVector &lhs,
  ...
  for (int i = 0; i < lhs.size_; i++) {
    temp.magnitudes_.get()[i] =
        lhs.magnitudes_.get()[i] + rhs.magnitudes_.get()[i];
  }
  return temp;
}
